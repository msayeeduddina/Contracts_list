...
let owner, fixedStaking, pool, treasury, mockOracle
...
before( async() => {
    ...
    masterChef  = await deployMockContract(owner, require('../abisForTests/MasterChefAbi.json'))
    await masterChef.mock.userInfo.withArgs(0, owner.address).returns(toWei(0), 0)

    autoBSW     = await deployMockContract(owner, require('../abisForTests/PoolAutoBsw.json'))
    ...
    await fixedStaking.setOracle(mockOracle.address, USDT_ADDRESS)
})
...
describe('Check fixed staking contract', async() => {
    it('Should add pool', async() => {
        ...
        for (let i = 0; i < 3; i++) await fixedStaking.addPool(pool)
        let tx = await fixedStaking.addPool(pool)
        GAS_REPORT.push(extractCost(await tx.wait(), {FUNC: 'addPool'}))
    })
    ...
    it('Should deposit', async() => {
        ...
        let tx = await fixedStaking.deposit(0, toWei(100))
        GAS_REPORT.push(extractCost(await tx.wait(), {FUNC: 'deposit'}))
    })
    ...
    it('Should harvest', async() => {
        ...
        let tx = await fixedStaking.harvest(0)
        GAS_REPORT.push(extractCost(await tx.wait(), {FUNC: 'harvest'}))
    ...
    it('Should withdraw', async() => {
        ...
        GAS_REPORT.push(extractCost(await tx.wait(), {FUNC: 'withdraw'}))
        const AfterBalance = +await BSW.balanceOf(owner.address) / 1e18
        const AfterInterest = await extractInterest(await fixedStaking.getUserInfo(owner.address))
