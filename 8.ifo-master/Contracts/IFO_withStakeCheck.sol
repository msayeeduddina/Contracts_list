...
contract IFO is ReentrancyGuard, Ownable {
    ...
    function depositPool(uint256 _amount, uint8 _pid) external nonReentrant notContract {
        ...
        //Check that user has staked BSW in long term pool
        require(_checkMinStakeAmount(msg.sender, _pid), "Not enough BSW in long term staking pool");
    ...
    function setPool(
        ...
        _poolInfo[_pid].stakeRequirement = _stakeRequirement;
    ...
    function setNewAutoBsw(address _pool) external onlyOwner {
        require(_pool != address(0), "Cannt be zero address");
        autoBsw = IAutoBsw(_pool);
    }
    ...
    function _checkMinStakeAmount(address _user, uint _pid) internal view returns (bool) {
        require(_pid < numberPools, "_pid out of bound");
